...
func DeriveMasterKey(cfg MasterKeyConfig) (keyout []byte, saltout []byte, err error) {
	...
	key := argon2.IDKey([]byte(cfg.Mpassword), salt, 1, 64*1024, 4, 32)
	//// Derive a 256-bit/32byte encryption key from the master password using Argon2id.
	// This uses 1 iteration, 64MB of memory, and 4 threads for resistance against brute-force and GPU attacks.
	// The salt ensures that identical passwords produce unique keys.
	return key, salt, nil
}

func Encryption(pass []byte, mKey []byte) (enPass []byte, nonceout []byte, err error) {
	...
	ciphertext := aesgcm.Seal(nil, nonce, pass, nil) // encrypt plain text
